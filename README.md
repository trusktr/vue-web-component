...
Because otherwise Vue components can only be used inside other Vue components.
This tool allows you to use Vue-based components outside of a Vue context, for
example in the regular DOM, and in other applications that use Angular, React,
Ember, or other HTML frameworks.

Plus, it's just easy to write Web Components this way!

Web Components (Custom Elements and related concepts) are elements that the
browser understands just like any other element, without the need for an HTML
library like Vue, Angular, or React. By making Custom Elements your component
will be consumable in any sort of application regardless of the view layer. You
can even render Custom Elements on the server-side, they're just HTML.

vue-web-component takes it one step further by using a Vue component to render
the content of you Custom Element. Behind the scenes,
[SkateJS](http://skatejs.netlify.com/) is used as a helper for wiring up the
functionality. SkateJS provides a set of powerfully simple mixins that make
defining custom elements super clean and convenient, on top of the browser's
native Web Components APIs.

#### Pros:

1. Using Vue makes it easy to describe how the DOM content of your component will change.
...
See here for details on [why `vue-custom-element` didn't meet those
